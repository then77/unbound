...
const itemVariants = cva(
  'relative flex flex-row items-center gap-2 rounded-lg p-2 text-start text-fd-muted-foreground wrap-anywhere [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        link: 'transition-colors hover:bg-fd-accent/50 hover:text-fd-accent-foreground/80 hover:transition-none data-[active=true]:bg-c-background data-[active=true]:text-c-primary data-[active=true]:hover:transition-colors',
        button:
...
function getItemOffset(depth: number) {
  return `calc(${2.5 + 3 * depth} * var(--spacing))`;
}
...
function SidebarSeparator({ className, style, children, ...props }: ComponentProps<'p'>) {
  ...
  return (
    <Base.SidebarSeparator
      className={cn(
        'inline-flex items-center gap-2 mb-2 px-2 mt-6 empty:mb-0 [&_svg]:size-4 [&_svg]:shrink-0 font-medium',
        depth === 0 && 'first:mt-0',
        className,
      )}
      style={{
